...
export default function ServicePage({ params }: { params: { slug: string } }) {
  const service = params?.slug && services.find((service) => service.slug === params.slug)
              ...
              {relatedServices.map((relatedService) => (
                <Card key={relatedService.id} className="overflow-hidden border-none shadow-md">
                  <CardContent className="p-6">
                    ...
                    <p className="text-muted-foreground line-clamp-3 mb-4">{relatedService.description}</p>
